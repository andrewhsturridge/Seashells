...
static inline void ledsAllColor(uint8_t r,uint8_t g,uint8_t b){
  ...
  // 4 .show() calls back-to-back; close enough to look simultaneous
...
static void blinkStart(uint8_t color, uint16_t on_ms, uint16_t off_ms, uint8_t reps=3){
  ...
  blink.on_ms = on_ms;
  blink.off_ms= off_ms;
  blink.remaining = reps;  // we'll count down on each ON->OFF transition
...
void side_setScene(uint16_t ids[4]) {
  for (int i = 0; i < 4; ++i) {
    ...
    // Set path and compute gain: master * per-clip (dB -> Q15)
    ch[i].path    = cm->path;
...
void side_playSlot(uint8_t slot) {
  slot &= 3;
  ch[slot].state = PLAYING; ch[slot].idx=0;
  if (!ch[slot].useRAM) { ch[slot].sd.cur = 0; if (ch[slot].sd.f) ch[slot].sd.f.seek(ch[slot].sd.dataStart); }
}
void side_ledAllWhite() {
  blinkStop();                 // cancel any in-progress blink
  ledsAllColor(255, 255, 255); // solid white
}
void side_blinkAll(uint8_t color, uint16_t on_ms, uint16_t off_ms) {
  blinkStart(color, on_ms, off_ms, /*reps*/3); // schedule non-blocking blink
}
void side_setGameMode(bool en){ gameMode=en; }
void side_startLoopAll(){
  for (int i=0;i<4;i++){
    ch[i].state = LOOPING; ch[i].idx = 0;
    if (!ch[i].useRAM) { ch[i].sd.cur = 0; if (ch[i].sd.f) ch[i].sd.f.seek(ch[i].sd.dataStart); }
  }
}
void side_stopAll(){
  for (int i=0;i<4;i++){
...
void setup() {
  ...
  masterGainQ15 = q15_from_db(MASTER_GAIN_DB);  // e.g., -40 dB

  // Buttons (external pull-ups)
  ...
  // Manifest + optional precache to PSRAM
  if (!Manifest_load()) Serial.println("[WARN] No manifest loaded");
  Manifest_precacheAll();

  // Assign default paths to channels (won't be used in GAME_MODE, but safe)
  for (int i=0;i<4;i++){ ch[i].path=""; ch[i].state=IDLE; ch[i].useRAM=false; ch[i].idx=0; }
  ...
  for (int i=0;i<4;++i) {
    ...
    if ((now - lastChangeMs[i]) > DEBOUNCE_MS) {
      if (pressed[i] != raw) {
        ...
        if (pressed[i]) {
          if (!gameMode) {
            // In non-game test mode, quickly flash and play slot
            ledWhite(i);
            side_playSlot(i);
          } else {
            // Game mode: just report
